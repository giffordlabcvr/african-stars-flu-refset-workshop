...
### 1.3.1. Run Nextclade on your GISAID FASTA

Download the H3N2-HA dataset (cached under `~/.nextclade`):
...
docker run --rm -it \
  ...
  nextclade run \
    ...
    /data/gisaid_ha.fasta
```

✅ **Checkpoint:** You should now have a file `nextclade.tsv` with QC + clade information per sequence.

* * * * *

### 1.3.2. Clean GISAID metadata
