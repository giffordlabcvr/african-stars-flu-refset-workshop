...
We'll use the **H3N2 HA** dataset. First download the dataset (cached under `~/.nextclade`), then run QC.
...
```
# Minimal metadata (accession, collection_date, geo_loc_name)
...
```
# Join via Docker + Python stdlib (no extra installs)
...
```
# Derive date/year/month from collection_date
docker run --rm -i -v "$PWD":/data -w /data nextstrain/base \
  python3 - <<'PY'
import csv, re
...
```
# Add 'strain' as the first column = accession (so Augur/Auspice can match tips)
...
### 1.2.5. (Option A) Use your **master reference** for alignment

If you are using a pre-chosen master reference (e.g. **CY033009.1**):
...
### 1.2.6. Subsample with Augur, align to reference, build tree

> We use `nextstrain/base` image. 
